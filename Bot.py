...
keycode = "sk-Sg5n37wt1ZBVJQShkOVStIUO45Mc16YvRmxoyq3n7xF5E72h"
...
@bot.message_handler(func=lambda message: message.text in ["Search and Replace", "Звичайна генерація", "SDXL diffusion", "Stable Cascade", "SDXL-Lightning"])
...
def sdxllightning(message):
  ...
  # Перевірка залишку генерацій
    ...
    try:
      ...
      bot.send_photo(message.chat.id, url, caption=f"💬Prompt:` {prompt}\n`\n💰У вас залишилося ***{generations[user_id] - 1}*** генерацій.", parse_mode="Markdown")
...
def handle_search_and_replace(message):
  ...
  # Зберігаємо фото з назвою ./husky-in-a-field.png
    ...
    bot.send_message(message.chat.id, "Введіть назву об'єкта який ви хочете замінити👇")
    bot.register_next_step_handler(message, reg_promt_img)
  except Exception as e:
    bot.send_message(message.chat.id, "Помилка при отриманні фото. Спробуйте ще раз.")  
...
def check_generations(message):
    ...
    bot.send_message(message.chat.id, f"💰Наразі у вас на балансі: ***{generations[user_id]}*** генерацій." , parse_mode="Markdown")
# Реєстрація команд
...
def promo_handler(message: types.Message):
    ...
    if generations[user_id] > genaratus:
      bot.send_message(message.chat.id, f"Ви не можите активувати ніякі промокоди, тому що: промокоди відновлюють генерації зображень, а у вас наразі більше ніж ***{genaratus}*** генерацій.", parse_mode="Markdown")
      return
    try:
...
def send_awesome(message):
  ...
  if idus == text:
    ...
    if user_id in all_users:
      ...
      try: 
        bot.send_message(text, "Користувач перейшов по вашому посиланню, ви отримали ***10*** генерацій.", parse_mode="Markdown")
        typeint = int(text)
